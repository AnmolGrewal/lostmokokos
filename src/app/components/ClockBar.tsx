...
const ClockBar = () => {
  ...
  const calculateCountdowns = () => {
    ...
    const hoursUntilUpdate = nextUpdateDate
      .subtract(daysUntilUpdate, "days")
      .diff(now, "hours");
    const nextUpdate = `${daysUntilUpdate > 0 ? daysUntilUpdate + "d " : ""}${
      hoursUntilUpdate > 0 ? hoursUntilUpdate + "h" : ""
    }`;
    ...
    const dailyReset = `${nextDailyReset.diff(now, "hours")}h ${
      nextDailyReset.diff(now, "minutes") % 60
    }m`;

    const nextWeeklyReset = moment()
      .tz("America/Los_Angeles")
      .startOf("week")
      .add(3, "days");
    if (now.isAfter(nextWeeklyReset)) nextWeeklyReset.add(1, "week");
    const daysUntilWeeklyReset = nextWeeklyReset.diff(now, "days");
    const hoursUntilWeeklyReset = nextWeeklyReset
      .subtract(daysUntilWeeklyReset, "days")
      .diff(now, "hours");
    const minutesUntilWeeklyReset = nextWeeklyReset
      .subtract(hoursUntilWeeklyReset, "hours")
      .diff(now, "minutes");
    const weeklyReset = `${
      daysUntilWeeklyReset > 0 ? daysUntilWeeklyReset + "d " : ""
    }${
      hoursUntilWeeklyReset > 0 ? hoursUntilWeeklyReset + "h " : ""
    }${minutesUntilWeeklyReset}m`;
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 h-7 items-center justify-center transition bg-primary-background-color">
      ...
      <div className="text-center text-sm md:text-base hidden md:block">
        <span className="text-primary-text-label-color">
          Echidna Release Date:{" "}
          <span className="font-semibold text-primary-text-label-color">
            {times.nextUpdate}
          </span>
        </span>
      </div>
      ...
      <div className="text-center text-sm md:text-base">
        <FontAwesomeIcon
          icon={faSun}
          className="text-image-sun-color text-xs md:text-sm mr-1"
        />
        <span className="text-primary-text-label-color">
          Daily Reset:{" "}
          <span className="font-semibold text-primary-text-label-color">
      ...
      <div className="text-center text-sm md:text-base hidden md:block">
        <FontAwesomeIcon
          icon={faCalendarWeek}
          className="text-image-calendar-color text-xs md:text-sm mr-1"
        />
        <span className="text-primary-text-label-color">
          Weekly Reset:{" "}
          <span className="font-semibold text-primary-text-label-color">

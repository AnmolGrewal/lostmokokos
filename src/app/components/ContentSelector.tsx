...
const ContentSelector = ({ currentPath }: { currentPath: string }) => {
  ...
  const contentItems: Raid[] = raidsInfo.filter(
    (raid) => !raid.path.endsWith("-hard"),
  );

  useEffect(() => {
    const container = scrollContainerRef.current;
    if (container) {
      const adjustedPath = currentPath.endsWith("-hard")
        ? currentPath.replace("-hard", "")
        : currentPath;
  ...
  return (
    <div className="relative size-full bg-primary-background-color">
      <div
        className="flex flex-shrink-0 overflow-x-auto scroll-smooth p-4 pl-8 justify-evenly"
          ...
          <Link
            ...
            className={clsx(
              ...
              currentPath !== item.path &&
                currentPath !== `${item.path}-hard` &&
                "hover:bg-primary-background-hover-color hover:scale-105",
              index !== 0 && "ml-4",
            )}
          >
            <img
            ...
            />
            <span className="text-center block text-sm font-bold text-chip-text-color">
              {item.label}
            </span>
          </Link>
        ))}
